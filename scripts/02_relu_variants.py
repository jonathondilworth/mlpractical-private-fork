...
for k in ['error(train)']:
    ax_1.plot(np.arange(1, all_stats_02[1].shape[0]) * stats_interval, 
              all_stats_02[1][1:, keys[k]], label='Training Error')

for k in ['error(valid)']:
    ax_1.plot(np.arange(1, all_stats_02[2].shape[0]) * stats_interval, 
              all_stats_02[1][1:, keys[k]], label='Validation Error')
...
for k in ['error(train)']:
    ax_2.plot(np.arange(1, all_stats_02[2].shape[0]) * stats_interval, 
              all_stats_02[2][1:, keys[k]], label='Training Error')

for k in ['error(valid)']:
    ax_2.plot(np.arange(1, all_stats_02[2].shape[0]) * stats_interval, 
              all_stats_02[2][1:, keys[k]], label='Validation Error')
...
for k in ['error(train)']:
    ax_3.plot(np.arange(1, all_stats_02[3].shape[0]) * stats_interval, 
              all_stats_02[3][1:, keys[k]], label='Training Error')

for k in ['error(valid)']:
    ax_3.plot(np.arange(1, all_stats_02[3].shape[0]) * stats_interval, 
              all_stats_02[3][1:, keys[k]], label='Validation Error')
...
for k in ['error(train)']:
    ax_4.plot(np.arange(1, all_stats_02[4].shape[0]) * stats_interval, 
              all_stats_02[4][1:, keys[k]], label='Training Error')

for k in ['error(valid)']:
    ax_4.plot(np.arange(1, all_stats_02[4].shape[0]) * stats_interval, 
              all_stats_02[4][1:, keys[k]], label='Validation Error')
...
fig_1.text(0.04, 0.5, 'Error', va='center', rotation='vertical', fontsize=16)

plt.suptitle("ReLU Variant Activation Functions Training and Validation Error, Step Size = 0.2", fontsize=16)

plt.savefig('02_relu_variants_error_svg.svg', dpi=None, facecolor='w', edgecolor='w',
        orientation='portrait', papertype=None, format='svg',
        transparent=False, bbox_inches=None, pad_inches=0.1,
        frameon=None)

plt.savefig('02_relu_variants_error_png.png', dpi=None, facecolor='w', edgecolor='w',
