...
#for k in ['acc(valid)']:
#    ax_2.plot(np.arange(1, all_stats_01[0].shape[0]) * stats_interval, 
#              all_stats_01[0][1:, keys[k]], label='Sigmoid')

for k in ['acc(valid)']:
    ax_2.plot(np.arange(1, all_stats_01[1].shape[0]) * stats_interval, 
              all_stats_01[1][1:, keys[k]], label='ReLU')

for k in ['acc(valid)']:
    ax_2.plot(np.arange(1, all_stats_01[2].shape[0]) * stats_interval, 
              all_stats_01[2][1:, keys[k]], label='LReLU')

for k in ['acc(valid)']:
    ax_2.plot(np.arange(1, all_stats_01[3].shape[0]) * stats_interval, 
              all_stats_01[3][1:, keys[k]], label='ELU')

for k in ['acc(valid)']:
    ax_2.plot(np.arange(1, all_stats_01[4].shape[0]) * stats_interval, 
              all_stats_01[4][1:, keys[k]], label='SELU')
...
#for k in ['acc(valid)']:
#    ax_3.plot(np.arange(1, all_stats_02[0].shape[0]) * stats_interval, 
#              all_stats_02[0][1:, keys[k]], label='Sigmoid')

for k in ['acc(valid)']:
    ax_3.plot(np.arange(1, all_stats_02[1].shape[0]) * stats_interval, 
              all_stats_02[1][1:, keys[k]], label='ReLU')

for k in ['acc(valid)']:
    ax_3.plot(np.arange(1, all_stats_02[2].shape[0]) * stats_interval, 
              all_stats_02[2][1:, keys[k]], label='LReLU')

for k in ['acc(valid)']:
    ax_3.plot(np.arange(1, all_stats_02[3].shape[0]) * stats_interval, 
              all_stats_02[3][1:, keys[k]], label='ELU')

for k in ['acc(valid)']:
    ax_3.plot(np.arange(1, all_stats_02[4].shape[0]) * stats_interval, 
              all_stats_02[4][1:, keys[k]], label='SELU')
...
#for k in ['acc(valid)']:
#    ax_4.plot(np.arange(1, all_stats_03[0].shape[0]) * stats_interval, 
#              all_stats_03[0][1:, keys[k]], label='Sigmoid')

for k in ['acc(valid)']:
    ax_4.plot(np.arange(1, all_stats_03[1].shape[0]) * stats_interval, 
              all_stats_03[1][1:, keys[k]], label='ReLU')

for k in ['acc(valid)']:
    ax_4.plot(np.arange(1, all_stats_03[2].shape[0]) * stats_interval, 
              all_stats_03[2][1:, keys[k]], label='LReLU')

for k in ['acc(valid)']:
    ax_4.plot(np.arange(1, all_stats_03[3].shape[0]) * stats_interval, 
              all_stats_03[3][1:, keys[k]], label='ELU')

for k in ['acc(valid)']:
    ax_4.plot(np.arange(1, all_stats_03[4].shape[0]) * stats_interval, 
              all_stats_03[4][1:, keys[k]], label='SELU')
...
ax_4.legend(loc=0)
ax_4.set_title('Step Size = 0.3')

fig_1.text(0.5, 0.04, 'Epoch number', ha='center', fontsize=16)
fig_1.text(0.04, 0.5, 'Validation accuracy', va='center', rotation='vertical', fontsize=16)

plt.suptitle("MNIST Validation Accuracy using ReLU Variant Activation Functions", fontsize=16)

plt.savefig('validation_acc_sens_analysis_svg.svg', dpi=None, facecolor='w', edgecolor='w',
        orientation='portrait', papertype=None, format='svg',
        transparent=False, bbox_inches=None, pad_inches=0.1,
        frameon=None)

plt.savefig('validation_acc_sens_analysis_png.png', dpi=None, facecolor='w', edgecolor='w',
