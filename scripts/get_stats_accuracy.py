...
for k in ['acc(train)']:
    ax_1.plot(np.arange(1, five_layers.shape[0]) * stats_interval, 
              five_layers[1:, keys[k]], label='Training accuracy')

for k in ['acc(valid)']:
    ax_1.plot(np.arange(1, five_layers.shape[0]) * stats_interval, 
              five_layers[1:, keys[k]], label='Validation accuracy')
...
for k in ['acc(train)']:
    ax_2.plot(np.arange(1, six_layers.shape[0]) * stats_interval, 
              six_layers[1:, keys[k]], label='Training accuracy')

for k in ['acc(valid)']:
    ax_2.plot(np.arange(1, six_layers.shape[0]) * stats_interval, 
              six_layers[1:, keys[k]], label='Validation accuracy')
...
for k in ['acc(train)']:
    ax_3.plot(np.arange(1, seven_layers.shape[0]) * stats_interval, 
              seven_layers[1:, keys[k]], label='Training accuracy')

for k in ['acc(valid)']:
    ax_3.plot(np.arange(1, seven_layers.shape[0]) * stats_interval, 
              seven_layers[1:, keys[k]], label='Validation accuracy')
...
for k in ['acc(train)']:
    ax_4.plot(np.arange(1, eight_layers.shape[0]) * stats_interval, 
              eight_layers[1:, keys[k]], label='Training accuracy')

for k in ['acc(valid)']:
    ax_4.plot(np.arange(1, eight_layers.shape[0]) * stats_interval, 
              eight_layers[1:, keys[k]], label='Validation accuracy')
